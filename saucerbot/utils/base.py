...
TEMPLATES_DIR: Path = (
    settings.BASE_DIR / "saucerbot" / "resources" / "elasticsearch" / "templates"
)
...
class BrewsLoaderUtil:
    ...
    def update_templates(self) -> None:
        ...
        for template in TEMPLATES_DIR.iterdir():
            ...
            if ext != "json":
                logger.warning(
                    "Located a non-json template file: %s. Ignoring.", template
                )
                continue
    ...
    @staticmethod
    def get_brews(
        session: requests.Session, location: str, store_id: str
    ) -> Iterable[Brew]:
...
class BrewsSearchUtil:
    ...
    def brew_info(self, search_term: str) -> str:
        ...
        message = (
            f"{total_hits} match{'' if total_hits == 1 else 'es'} "
